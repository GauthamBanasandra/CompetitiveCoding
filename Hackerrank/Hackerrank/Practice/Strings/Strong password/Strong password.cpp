...
int get_missing_strength(const std::string &line) {
  ...
  for (auto const &c : line) {
    ...
    switch (classifier[c]) {
      case DIGIT:criteria.reset(DIGIT);
        break;

      case LOWER_CASE:criteria.reset(LOWER_CASE);
        break;

      case UPPER_CASE:criteria.reset(UPPER_CASE);
        break;

      case SPECIAL_CHAR:criteria.reset(SPECIAL_CHAR);
        break;
