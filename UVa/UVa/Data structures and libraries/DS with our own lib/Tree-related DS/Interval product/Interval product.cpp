...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        ds::bin_tree::seg_tree<std::vector<int>, int, int> seg_tree(seq, seq.size(),
                                                                    access_data<std::vector<int>, int>,
                                                                    merge_nodes,
                                                                    update_data<int, int>);

        while (k-- > 0) {
            std::getline(std::cin, line);
            std::istringstream cmd_tokenizer(line);
            cmd_tokenizer >> cmd >> a >> b;

            switch (cmd) {
                case 'C':
                    seg_tree.update_point(a - 1, b);
                    break;

                case 'P': {
                    const auto response = seg_tree.query(ds::bin_tree::range{a - 1, b - 1});
                    if (response.data > 0) {
                        product += "+";
