...
enum Direction {
    k_north, k_south, k_east, k_west, k_north_east, k_north_west, k_south_east, k_south_west, k_last
};

struct Pos {
    Pos(bool is_valid, int row, int column) : is_valid(is_valid), row(row), column(column) {}
...
class BoggleBlitz {
public:
    BoggleBlitz(int n, std::vector<std::string> grid) : n(n), grid(std::move(grid)) {}
...
private:
    ...
    void NextWord(Pos &pos, std::string &word, BoggleBlitzLayout &layout, LegalWordsContainer &legal_words);
...
inline Pos BoggleBlitz::GetNextPos(const Pos &pos, Direction d) {
    ...
    next_pos.is_valid = (next_pos.row < n && next_pos.row >= 0) && (next_pos.column < n && next_pos.column >= 0);
    return next_pos;
}
...
void BoggleBlitz::NextWord(Pos &pos, std::string &word, BoggleBlitzLayout &layout, LegalWordsContainer &legal_words) {
    if (word.length() >= 3) {
        legal_words.insert(word);
    }

    for (int direction = Direction::k_north; direction != Direction::k_last; ++direction) {
        auto next_pos = GetNextPos(pos, static_cast<Direction>(direction));
        if (!next_pos.is_valid || !IsLegal(pos, next_pos) || layout.IsVisited(next_pos)) {
            continue;
        }
...
int main() {
    ...
    for (int i = 0; i < t; ++i) {
        ...
        auto legal_words = BoggleBlitz(n, grid).Solve();
        if (i > 0) {
            std::cout << std::endl;
        }
